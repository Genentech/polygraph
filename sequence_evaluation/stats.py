...
def groupwise_fishers(data, reference_group, val_col, reference_val=None, group_col="Group"):
    """
    Pairwise fisher's exact test for proportions between each non-reference group and the reference group

    Args:
        data (pd.DataFrame, anndata.AnnData): Pandas dataframe with group IDs and values to compare, or anndata object containing this dataframe in .obs
...
def groupwise_mann_whitney(data, val_col, reference_group, group_col="Group"):
    """
    Comparing the mean values between each non-reference group and the reference group using the 
    Mann-Whitney U test
...
def kruskal_dunn(data, val_col, group_col="Group"):
    """
    Comparing the mean values between all groups using the Kruskal-Wallis test followed by Dunn's post-hoc test
    ...
    # Kruskal-Wallis test
    pval = kruskal(*[data.loc[data[group_col] == group, val_col] for group in groups]).pvalue

    # Dunn's test
    padj = posthoc_dunn(data, val_col=val_col, group_col=group_col, p_adjust="bonferroni")
