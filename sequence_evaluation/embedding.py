...
def embedding_pca(ad, **kwargs):
    ...
    """
    sc.pp.pca(ad, **kwargs)
    print("Fraction of variance explained: ", np.round(ad.uns['pca']['variance_ratio'], 2))
    print("Fraction of total variance explained: ", np.sum(ad.uns['pca']['variance_ratio']))
    return ad
...
def differential_analysis(ad, reference_group, group_col='Group'):
    """
    Perform Wilcoxon rank-sum test on sequence embeddings to find features enriched/depleted
    ...
    """
    # Perform test
    sc.tl.rank_genes_groups(ad, groupby=group_col, groups='all',
                            reference=reference_group, rankby_abs=True, method='wilcoxon')
...
def one_nn_stats(ad, reference_group, group_col='Group', use_pca=False):
    """
    Calculate the following 1-nearest neighbor statistics based on the sequence embeddings:
    ...
    Returns:
        ad (anndata.AnnData): Modified anndata object containing index, distance and group ID of
        each sequence's nearest neighbor in .obs, as well as a probability table of nearest neighbor groups for each group
        in .uns
    """
    
    # Get nearest neighbor for each sequence
    if use_pca:
        nbrs = NearestNeighbors(n_neighbors=2, algorithm='ball_tree').fit(ad.obsm['X_pca'])
    ...
    ad.obs.loc[:, 'one_nn_group'] = ad.obs[group_col].iloc[ad.obs['one_nn_idx'].tolist()].tolist()

    # Normalized count table of nearest neighbor groups
    one_nn_group_df = ad.obs[[group_col, 'one_nn_group']].value_counts().reset_index().pivot_table(
    index=group_col, columns='one_nn_group', values='count').fillna(0).astype(int)
    ...
    ad.uns["1NN_ref_prop_test"] = groupwise_fishers(ad.obs, val_col='one_nn_group', reference_group=reference_group, reference_val=reference_group, group_col=group_col)
...
def within_group_knn_dist(ad, n_neighbors=10, group_col='Group', use_pca=False):
    ...
    for group in groups:
        ...
        nbrs = NearestNeighbors(n_neighbors=n_neighbors+1, algorithm='ball_tree').fit(group_X)
    ...
    if len(groups) == 2:
        ad.uns["knn_dist_test"] = groupwise_mann_whitney(ad.obs, val_col='KNN Distance', reference_group=groups[0], group_col=group_col)
    elif len(groups) > 2:
        ad.uns["knn_dist_test"] = kruskal_dunn(ad.obs, val_col='KNN Distance', group_col=group_col)
...
def dist_to_reference(ad, reference_group, group_col='Group', use_pca=False):
    ...
    Returns:
        ad (anndata.AnnData): Modified anndata object containing distance to reference in .obs
    """
    ...
    for group in groups:
        ...
        # Get pairwise euclidean distance of each group sequence to each reference sequence
        ...
        if group != reference_group:
            ...
            # If the group is the reference group, the nearest neighbor will be the sequence
            # itself. So we find the next nearest neighbor.
            dlist = []
    ...
    # Mann-whitney or Kruskal-wallis test
    if len(groups) == 2:
        ad.uns["ref_dist_test"] = groupwise_mann_whitney(ad.obs, val_col='Distance to closest reference', reference_group=reference_group, group_col=group_col)
    elif len(groups) > 2:
        ad.uns["ref_dist_test"] = kruskal_dunn(ad.obs, val_col='Distance to closest reference', group_col=group_col)

    return ad


def embedding_analysis(matrix, seqs, reference_group, group_col='Group', n_neighbors=15, use_pca=False):
    """
    A single function to calculate all embedding distance-based metrics.

    Args:
        matrix (np.array, pd.DataFrame): A probability table or embedding matrix of dimensions seqs x features
    ...
    Returns:
        ...
        Wilcoxon test comparing feature abundance in nonreference vs. reference groups (in .uns["DE_test"])
        Index, distance and group of each sequence's nearest neighbor (in .obs)
        Test for proportion of sequences in each group whose nearest neighbor is in the reference group (.uns["1NN_reference_prop_test"])
        Probability table of nearest neighbor groups for each group (.uns["1NN_group_probs"])
        ...
        Distance to the closest reference sequence for each sequence (.obs['Distance to closest reference'])
        Test for between group difference in Distance to the closest reference sequence (.uns["ref_dist_test"])
    """ 
